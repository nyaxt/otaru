...
func Install(srv *mgmt.Server, s *scheduler.Scheduler, bs blobstoregc.GCableBlobStore, idb inodedb.DBFscker) {
	...
	rtr.HandleFunc("/trigger", func(w http.ResponseWriter, req *http.Request) {
		...
		jv := s.RunImmediatelyBlock(&blobstoregc.GCTask{bs, idb, dryrun})
